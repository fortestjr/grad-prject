...
def print_results(target: str, protocol: str, port_results: List[Dict]):
    """Print the port test results to the terminal."""
    print(f"\nFirewall Test Report for {target} ({protocol.upper()})")
    print("-" * 50)
    if port_results:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if port_results:
            print_results(args.target, args.protocol, port_results)
        else:
            print("No ports were tested.")
    except ValueError:
        print("Error: Invalid port list. Please enter comma-separated port numbers.")
    except Exception as e:
